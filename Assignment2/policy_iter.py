...
def compute_vpi(pi, mdp, gamma):
    ...
    for state in range(mdp.nS):
        for result in mdp.P[state][pi[state]]:
            P[state][result[1]] += result[0]
            rewards[state] += result[2] * result[0]
    V = np.linalg.solve(np.eye(mdp.nS)-gamma * P, rewards)
    return V
...
def compute_qpi(vpi, mdp, gamma):
    ...
    for state in range(mdp.nS):
        for action in range(mdp.nA):
            Qpi[state][action] = np.sum([result[0] * (result[2] + gamma * vpi[result[1]]) for result in mdp.P[state][action]])
    return Qpi
