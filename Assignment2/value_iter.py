...
def value_iteration(mdp, gamma, nIt):
    ...
    for it in range(nIt):
        ...
        V, pi = np.array([None] * 16, dtype=float), np.array([None] * 16, dtype=float)
        for state in range(mdp.nS):
            action_results = []
            for action in range(mdp.nA):
                state_action = mdp.P[state][action]
                action_results.append(np.sum(
                    next_state[0] * (next_state[2] + gamma * Vprev[next_state[1]]) for next_state in state_action))
            V[state] = action_results[np.argmax(action_results)]
            pi[state] = np.argmax(action_results)
...
plt.plot(Vs_VI)
plt.legend([f"state {state}" for state in range(mdp.nS)])
