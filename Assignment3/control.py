...
state_i = cart_pole.get_state(state_tuple)
...
###### BEGIN YOUR CODE ######
val_func = np.random.uniform(low=0, high=0.10, size=(NUM_STATES,))
action_state_prob = np.ones((NUM_STATES, NUM_ACTIONS, NUM_STATES)) / NUM_STATES
rewards = np.zeros(NUM_STATES)
rewards_sum = np.zeros(NUM_STATES)
state_action_counter = np.zeros((NUM_STATES, NUM_ACTIONS, NUM_STATES))
state_action_tried = np.zeros((NUM_STATES, NUM_ACTIONS))
state_reach_counter = np.zeros(NUM_STATES)
###### END YOUR CODE ######
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    # model.
    ###### BEGIN YOUR CODE ######
    action_0_val = action_state_prob[state_i][0] @ val_func
    action_1_val = action_state_prob[state_i][1] @ val_func
    action = 1 if action_1_val > action_0_val else 0
    ###### END YOUR CODE ######
    ...
    state_action_counter[state_i][action][new_state] += 1
    state_action_tried[state_i][action] += 1
    # record the rewards for every `new_state`
    rewards_sum[new_state] += R
    # record the number of time `new_state` was reached
    state_reach_counter[new_state] += 1
    ###### END YOUR CODE ######
    ...
    if new_state == NUM_STATES - 1:
        ...
        for state_i in range(NUM_STATES):
            for action in range(NUM_ACTIONS):
                if state_action_tried[state_i][action] != 0:
                    action_state_prob[state_i][action] = state_action_counter[state_i][action] / state_action_tried[state_i][action]
            if state_reach_counter[state_i] != 0:
                rewards[state_i] = rewards_sum[state_i]/ state_reach_counter[state_i]
        ###### END YOUR CODE ######
        ...
        ###### BEGIN YOUR CODE ######
        next_step_val_iteration = np.zeros(val_func.shape)
        iter_num = 0
        while True:
            for state_i in range(NUM_STATES):
                val_for_action_0 = action_state_prob[state_i][0] @ (GAMMA * val_func)
                val_for_action_1 = action_state_prob[state_i][1] @ (GAMMA * val_func)
                next_step_val_iteration[state_i] = rewards[state_i] + np.max([val_for_action_0, val_for_action_1])
            if np.max(np.abs(next_step_val_iteration - val_func)) < TOLERANCE:
                if iter_num == 0:
                    consecutive_no_learning_trials += 1
                else:
                    consecutive_no_learning_trials = 0
                    val_func = next_step_val_iteration
                    break
            iter_num += 1
            val_func = next_step_val_iteration
