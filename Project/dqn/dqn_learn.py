...
def dqn_learing(
    ...
    ######
    Q = q_func(input_arg, num_actions=num_actions)
    Q.train()
    Q_target = q_func(input_arg, num_actions=num_actions)
    ...
    for t in count():
        ...
        action = select_epilson_greedy_action(Q, recent_frames, t)
        obs, reward, done, info = env.step(action)
        ...
        if (t > learning_starts and
            ...
            q_pred = Q.forward(torch.from_numpy(states / 255).float())[indexes, actions]
            q_next = Q_target.forward(torch.from_numpy(next_states / 255).float()).max(dim=1)[0]
            q_next[dones == 1] = 0.0
            q_target = torch.IntTensor(rewards) + (gamma * q_next) - q_pred
            error = torch.clip(q_target, -1, 1)
            q_pred.backward(-1 * error.data)
            optimizer.step()
            #####
        ...
        best_reward = max(episode_rewards[-100:]) if episode_rewards else best_reward
        Statistic["best_reward"].append(best_reward)
        ...
        if t % LOG_EVERY_N_STEPS == 0 and t > learning_starts:
            ...
            print("best reward %f" % best_reward)
